...
// =====================================================================
//  BOXMET AutoCAD Plugin — AutoCAD 2023/2024
//  Lệnh: BOXMET
//  Vẽ trải phẳng hộp kim loại tấm trực tiếp lên bản vẽ AutoCAD
//  Theo Mau_test.dxf: toàn bộ layer 0, base DASHED, slit màu 3
// =====================================================================
...
namespace BoxMetPlugin
{
    ...
    public static class DrawEngine
    {
                     ...
                     double filletR = 0.5)
        {
            try
            {
                ...
                // Điểm tiếp xúc (tangent points) — nơi arc chạm vào line
                ...
                // Vẽ line từ tangent point → đầu cuối (đã trim)
        ...
        public static void Draw(BoxParams p, BlockTableRecord btr, Transaction tr)
        {
            ...
            {
                // ═══ CASE 1: Length >= Width ═══
                // Bottom/Top lui T (3 nét, không đè base), Right/Left full

                // -- Bottom Flap (lui T) — 3 nét + fillet corners --
                ...
                // -- Top Flap (lui T) — 3 nét + fillet corners --
                ...
                // -- Slits tại 4 góc lui T (Bottom/Top) --
            ...
            {
                // ═══ CASE 2: Width > Length ═══
                // Bottom/Top full, Right/Left lui T (3 nét, không đè base)
                ...
                // -- Left Flap (lui T) — 3 nét + fillet corners --
    ...
    public class BoxMetCommand
    {
        ...
        public void RunBoxMet()
        {
            ...
            ed.WriteMessage("\n╔══════════════════════════════════════╗");
            ed.WriteMessage("\n║   BOXMET — Sheet Metal Flat Pattern   ║");
            ed.WriteMessage("\n╚══════════════════════════════════════╝\n");
            ...
            double R_outer = (p.Material.ToUpper() == "SS" && p.Thickness >= 6.0 && p.Thickness <= 9.0) ? 2.0 : 0.5;
            ed.WriteMessage($"\n>> BOXMET: Hoàn thành! L={p.Length} W={p.Width} T={p.Thickness}");
            ed.WriteMessage($"\n>> Mode: {(p.IsLongX ? "L≥W (Bottom/Top lui T)" : "W>L (Left/Right lui T)")}");
            ed.WriteMessage($"\n>> Layer 0, slit màu 3, outer corner R={R_outer}\n");
